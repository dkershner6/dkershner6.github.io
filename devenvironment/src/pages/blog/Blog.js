...
const Blog = (props) => {
  if (props.match.params.category !== undefined) {
    return (
      ...
      {posts.filter(post => post.categories.includes(props.match.params.category)).map((post, index) => (
  ...
  return (
    <LinkContainer to={`/blog/${props.post.handle}`}>
      <Jumbotron>
        ...
        <h6>{props.post.date} {props.post.categories.map((category, index) => (
          <Link to={`/blog/category/${category}`}><span class="badge badge-pill badge-info" key={index}>{category.toLowerCase()}</span></Link>
        ))}</h6>
        <img src={`/posts/images/${props.post.image}`} width="100%" alt={props.post.title} />
      </Jumbotron>
    </LinkContainer>
  );
};
