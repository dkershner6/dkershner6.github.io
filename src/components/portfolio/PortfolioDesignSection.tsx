...
import { Row, Col, Card, ListGroup, ListGroupItem, Collapse } from 'react-bootstrap';
...
const PortfolioDesignSection = (props: PortfolioDesignSectionProps) => {
    const { project, sectionId, sectionName, headerBg, headerText, getOpen, toggleOpen } = props;
    return (
        <Row id={props.sectionId} className="mt-3">
            <Col>
                <Card bg={headerBg} text={headerText}>
                    <Card.Header data-testid={`header-${sectionName}`} style={{ cursor: 'pointer' }} onClick={() => toggleOpen(sectionId)}>
                        <h3>{sectionName}</h3>
                    </Card.Header>
                    <Collapse data-testid={`collapse-${sectionName}`} in={getOpen(sectionId)}>
                        <Card.Body>
                            <Row>
                                {project[sectionId].map((dataSection, index) => (
                                    <Col key={index} className="mt-3" xs="12" xl={project[sectionId].length > 1 ? '6' : '12'}>
                                        <Card id={dataSection.id} bg="secondary" text="dark">
                                            <Card.Header style={{ cursor: 'pointer' }} onClick={() => toggleOpen(dataSection.id)}>
                                                <Row>
                                                    <Col>
                                                ...
                                                <ListGroup>
                                                    {dataSection.attributes.map((dataSectionAttribute, iindex) => (
                                                        <ListGroupItem
                                                            ...
                                                            <Row>
                                                                <Col xs="12" md="3">
                                                                    <strong className={dataSectionAttribute.name === 'Example' ? 'text-info' : ''}>
                                                                        {dataSectionAttribute.name}:
                                                                    </strong>
                                                                </Col>
                                                                <Col xs="12" md="9">
                                                                    <p className={dataSectionAttribute.name === 'Example' ? 'text-info' : ''}>
